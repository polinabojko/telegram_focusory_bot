...
def check_streak_reset(user_id):
    ...
    for habit_id, last_marked in habits_list:
...
def send_stats(bot, message):
    ...
    mood_map = {"😃":5,"🙂":4,"😐":3,"😔":2,"😡":1}
    if moods:
        avg_val = round(sum(mood_map.get(m,3) for m in moods)/len(moods))
        reverse_map = {v:k for k,v in mood_map.items()}
        avg_mood = reverse_map.get(avg_val,"—")
    else:
        avg_mood = "—"
...
"""
    ...
    bot.send_message(user_id, "Вы можете вернуться в главное меню:", reply_markup=markup)
