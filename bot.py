...
def save_mood(message):
    chat_id = str(message.chat.id)
    user_moods.setdefault(chat_id, {})[today_str()] = message.text
    save_data()
...
def pomodoro_finished(chat_id):
    ...
    text = "✅ Pomodoro done! Take a short break 🌿" if lang == "en" else "✅ Фокус завершен! Сделай перерыв 🌿"
...
def start_pomodoro(chat_id, minutes):
    lang = get_lang(chat_id)
    text = f"🍅 Focus started — {minutes} minutes." if lang == "en" else f"🍅 Фокус начался — {minutes} минут."
    bot.send_message(int(chat_id), text)
...
def stop_pomodoro(message):
    ...
    text = "🛑 Pomodoro stopped." if get_lang(chat_id) == "en" else "🛑 Pomodoro остановлен."
    bot.send_message(message.chat.id, text)
